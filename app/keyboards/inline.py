...
def get_genres_keyboard(selected: Set[str] = None) -> InlineKeyboardMarkup:
    """Клавиатура выбора жанров (с кнопкой «свой вариант»)."""
    ...
    builder.row(
        InlineKeyboardButton(text="✏️ Свой вариант", callback_data="genre:other")
    )
    builder.adjust(3)
    if selected:
        builder.row(
...
def get_mood_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура выбора настроения (+ свой вариант)."""
    builder = InlineKeyboardBuilder()
    for mood in MOODS:
        builder.button(
    ...
    builder.row(
        InlineKeyboardButton(text="✏️ Свой вариант", callback_data="mood:other")
    )
    builder.adjust(1)
    return builder.as_markup()
...
def get_when_listen_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура «когда чаще слушаешь музыку»."""
    builder = InlineKeyboardBuilder()
...
def get_chat_test_keyboard(bot_username: str, chat_id: int = 0) -> InlineKeyboardMarkup:
    """Кнопка «Пройти тест» — ведёт в ЛС бота с start=from_chat_{chat_id}."""
    builder = InlineKeyboardBuilder()
