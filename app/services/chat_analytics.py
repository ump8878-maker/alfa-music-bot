# Музыкальный сканер чата: агрегация статистики, профиль, комментарии
...
from app.models import ChatMember, MusicProfile
from app.keyboards.data import GENRES


@dataclass
class ChatProfile:
    """Музыкальный портрет чата для вывода в /chat_scan."""
    profile_name: str
    genre_stats: List[tuple]  # [(name, pct), ...]
    top_artists: List[str]
    vibe_text: str
    overall_score: float


# Жанр id -> человекочитаемое название для сканера
...
async def collect_chat_music_stats(
    ...
    """
    Собирает агрегированную статистику по музыке в чате.
    Возвращает None, если участников с профилем < 3.
    """
    ...
    total_score = 0.0
    from app.rating import compute_user_taste_score
    for p in profiles:
        ...
        if p.listening_time:
            listening_counts[p.listening_time] = listening_counts.get(p.listening_time, 0) + 1
    ...
    dominant_listening = max(listening_counts.items(), key=lambda x: x[1])[0] if listening_counts else None
...
def _derive_profile_name(stats: Dict[str, Any]) -> str:
    """Определяет музыкальный архетип чата по статистике."""
    ...
    if any(g in ("electronic", "электроника", "techno", "техно") for g in top_genres):
        if listening == "night":
            return "Ночные меломаны"
        return "Электронный клуб"
    if any(g in ("indie", "инди") for g in top_genres):
        if mood == "melancholic":
            return "Инди-меланхолики"
        return "Инди-семья"
...
def _derive_vibe_text(stats: Dict[str, Any]) -> str:
    """Короткий текст «общий вайб» по настроению и времени прослушивания."""
    mood = stats.get("dominant_mood")
    listening = stats.get("dominant_listening")
    parts = []
    if listening == "night":
        parts.append("ночные прогулки")
        parts.append("поздние сеты")
...
async def calculate_chat_profile(
    ...
    """
    Считает музыкальный профиль чата (архетип, жанры, артисты, вайб).
...
def generate_chat_comment(chat_profile: ChatProfile) -> str:
    """
    Генерирует короткий шуточный комментарий на основе профиля чата.
    Использует список CHAT_SCAN_COMMENTS из utils/humor.
    """
    from app.utils.humor import get_chat_scan_comment
