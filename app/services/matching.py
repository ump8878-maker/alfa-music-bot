...
def calculate_rarity_score(
    ...
    """
    Рассчитывает редкость музыкального вкуса (для отображения и матчинга).
    0 = максимально мейнстрим, 1 = максимально редкий.
    ВАЖНО: не использовать для рейтинга чатов или пользователей — иначе появляется
    предвзятость в пользу «редких» жанров. Рейтинг строится только на активности,
    полноте квиза, согласованности и разнообразии (см. rating.py, chat_rating.py).
    """
